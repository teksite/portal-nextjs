...
export function searchLicensesByTitle(searchTerm: string): {
	...
	list.forEach((license: LicenseType) => {
		...
		const normalizedDescription = license.description
			? normalizeText(license.description)
			: "";
		const originalTitle = license.title;

		// Exact match
		if (
			normalizedTitle === normalizedTerm ||
			(license.code && normalizeText(license.code) === normalizedTerm)
		) {
			exact.push({
				...license,
