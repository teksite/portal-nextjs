...
export default function ThemeChangerProvider({ position = 'left', children }: ThemeChangerProviderProps) {
   ...
   return (
      <>
         ...
         <button
            className={`${positionClass} fixed  bottom-6 z-50 rounded-full bg-sky-900 p-3  shadow-lg transition hover:bg-blue-700`}
            onClick={() => setIsOpen(!isOpen)}>
            {hideImages ? 'تنظیمات':'⚙'}️
         </button>

         <div
            className={`fixed top-0 left-0 z-40 h-full w-80 transform rounded-s-2xl bg-zinc-50 dark:bg-zinc-900 shadow-lg transition-transform ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
