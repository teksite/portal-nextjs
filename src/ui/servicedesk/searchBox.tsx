...
export default function SearchService() {
    ...
    const searchTitleHandler = useCallback(
        debounce(async (term: string) => {
            ...
            try {
                if (term.length >= 4) {
                    const { exact, startsWith, containsWords, descriptionContains } = searchLicensesByTitle(term);
                    const allResults = [...exact, ...startsWith, ...containsWords, ...descriptionContains];
            ...
            <Form className="grid gap-6 lg:grid-cols-4" action="/servicedesk">
                <div className="relative lg:col-span-3">
                    ...
                    <input
                        ...
                        placeholder="جستجو..."
                        name="title"
                        className={`input-style ${showSuggestions && results.length ? 'rounded-b-none border-b-0 shadow' : ''}`}
                    ...
                    {showSuggestions && results.length > 0 && (
                        <div className="absolute top-full inset-x-0 z-10" ref={suggestionBoxRef}>
                            <ul className="x-box rounded-t-none divide-y divide-t-zinc-300 dark:divide-t-zinc-600 border-blue-600 border-t-zinc-300 dark:border-t-zinc-600 py-1 overflow-y-auto max-h-[500px]">
                                {isLoading ? (
                                    <li className="py-3 px-4 text-center text-gray-500">در حال بارگذاری...</li>
                                ) : (
