...
export function ListSemiColored({data, column}: ListColoredProps) {
  ...
  const CollapsedComponent = ({id, data: license, onExpand}: GenericItemCollapsedProps) => {
    ...
    return (
      <motion.div
        layoutId={`card-${id}`}
        onClick={() => onExpand(id)}
        className="border shadow-lg rounded-xl overflow-hidden cursor-pointer hover:bg-slate-50 border-zinc-300 hover:shadow-sm duration-150 transition-all ease-linear"
      >
        <div className="flex gap-3 items-stretch h-full">
          <LicenseIcon
            name={groupInfo.icon ?? "Sayer"}
            className={`h-full py-1 px-2 ${bg} ${fill}`}
          />
          <div className="p-3">
            <motion.h4
              id={`${id}-title`}
