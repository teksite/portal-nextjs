...
export function ListUp({data, column}: ListColoredProps) {
    ...
    const CollapsedComponent = ({id, data: license, onExpand}: GenericItemCollapsedProps) => {
        ...
        return (
            <motion.div
                layoutId={`card-${id}`}
                onClick={() => onExpand(id)}
                className={`border shadow-xl rounded-xl overflow-hidden cursor-pointer hover:bg-slate-50  hover:shadow-sm duration-150 transition-all ease-linear pt-3 ${border}`}
            >
                <div className="">
                    <LicenseIcon
                        name={groupInfo.icon ?? "Sayer"}
                        className={`h-full py-1 px-2 mx-auto ${fill}`}
                    />
                    <div className="p-3">
                        <motion.h4
                            id={`${id}-title`}
                            layoutId={`title-${id}`}
                            className={`block  text-center font-bold ${text}`}
                        >
                            {license.title}
                        </motion.h4>
                    </div>
