...
export function SearchInputTest({
   ...
   type = 'noButton',
   data
}: Pick<SearchInputProps<LicenseType>, 'size' | 'type' | 'data'>) {
   const [query, setQuery] = useState<string>();
   const [text, setText] = useState<string>('');
   const [selection, setSelection] = useState<LicenseType>();
...
function ListItem({ item: service, query }: { item: LicenseType; query?: string }) {
   return (
      <div className='flex'>
         <div className='flex-1'>
            {query ? (
               <HighlightText text={service.title} query={query} highlightClassName='font-bold text-blue-700' />
            ) : (
               service.title
            )}
         </div>
...
function filterLogic2(services: LicenseType[], query?: string): LicenseType[] {
   ...
   const beforeSort = services
      .map((service) => {
         ...
         if (
            !terms.some(
               // !terms.every(
               (t) => title.includes(t) || genre.includes(t) || description.includes(t)
            )
         ) {
            return null;
         }
